...
# else
    ...
    private string RequestBannersSDK(string bannerRequestJSON) { return ""; }
        ...
        GetScreenshotSDK(img, img.Length);
#else
...
#if (!UNITY_EDITOR)
		    RequestInviteUrlSDK(inviteLink);
#else
...
#if (!UNITY_EDITOR)
            RequestBannersSDK(CrazyJsonHelper.ToJson(banners));
#else
